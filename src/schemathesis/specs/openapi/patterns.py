...
def _classify_structure(nodes: list[_Node]) -> tuple[str, ...]:
    ...
    match nodes:
        ...
        case [(sre.AT, _) as leading, *parts, (sre.AT, _) as trailing] if all(
            op in _CONTENT_OPS for op, _ in parts
        ):
...
def _transform_anchored_single(
    leading: _Node, content: _Node, trailing: _Node,
    min_l: int | None, max_l: int | None,
) -> list[_Node] | None:
    """^content$ with canonicalization of [\\w\\W] -> . before transform."""
    op, value = content
    ...
    if op == sre.IN and _matches_anything(value):
        ...
        op in REPEATS
        and len(value[2]) == 1
        and value[2][0][0] == sre.IN
        and _matches_anything(value[2][0][1])
    ):
        content = (op, (value[0], value[1], [(sre.ANY, None)]))

    op, value = content
    if op == LITERAL and (
        (min_l is not None and min_l > 1) or (max_l is not None and max_l < 1)
    ):
        return None
...
def _transform_anchored_multi(
    leading: _Node, parts: list[_Node], trailing: _Node,
    min_l: int | None, max_l: int | None,
) -> list[_Node] | None:
