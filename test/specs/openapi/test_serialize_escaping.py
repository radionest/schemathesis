"""Escaping edge-case tests for the regex AST serializer.

Focuses on _serialize_literal and _serialize_literal_in_class — the most
fragile parts of the serializer where wrong escaping silently changes semantics.
"""
...
# ---------------------------------------------------------------------------
# Literal escaping (outside character classes)
# ---------------------------------------------------------------------------
...
def test_literal_metachar_escaped(char):
    """Regex metacharacters in LITERAL nodes must be escaped so they match literally."""
...
def test_literal_printable_roundtrip(char):
    """Every printable ASCII character round-trips through the serializer."""
    pattern = re.escape(char)
    parsed = sre_parse.parse(pattern)
    serialized = _serialize(list(parsed))
...
def test_literal_control_char(char, name):
    """Control and non-printable characters must serialize to valid escape sequences."""
    pattern = re.escape(char)
    parsed = sre_parse.parse(pattern)
    serialized = _serialize(list(parsed))
    compiled = re.compile(serialized)
...
def test_literal_unicode_char(char, name):
    """Unicode characters must round-trip through the serializer."""
    pattern = re.escape(char)
    parsed = sre_parse.parse(pattern)
    serialized = _serialize(list(parsed))
    compiled = re.compile(serialized)
...
# ---------------------------------------------------------------------------
# Character class escaping (inside [...])
# ---------------------------------------------------------------------------
...
def test_class_special_char(char, name):
    """Characters special inside [...] must be properly escaped."""
...
def test_class_metachar_inside_class(char):
    """Regex metacharacters inside character classes — some don't need escaping."""
    pattern = f"[{re.escape(char)}]"
    try:
        parsed = sre_parse.parse(pattern)
    except re.error:
...
def test_negated_class(pattern, should_match, should_not_match):
    """Negated character classes must preserve negation through serialization."""
    parsed = sre_parse.parse(pattern)
    serialized = _serialize(list(parsed))
    compiled = re.compile(serialized)
...
def test_range_in_class(pattern, samples_in, samples_out):
    """Character ranges must be preserved through serialization."""
    parsed = sre_parse.parse(pattern)
    serialized = _serialize(list(parsed))
    compiled = re.compile(serialized)
...
def test_not_literal(pattern, should_match, should_not_match):
    """NOT_LITERAL opcode (single char negation) must round-trip correctly."""
    parsed = sre_parse.parse(pattern)
    serialized = _serialize(list(parsed))
    compiled = re.compile(serialized)
...
# ---------------------------------------------------------------------------
# Combined patterns with tricky escaping
# ---------------------------------------------------------------------------
...
def test_escaped_in_context(pattern, should_match):
    """Escaped metacharacters in concatenated patterns."""
    parsed = sre_parse.parse(pattern)
    serialized = _serialize(list(parsed))
    compiled = re.compile(serialized)
    assert compiled.fullmatch(should_match), (
